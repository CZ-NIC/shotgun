...
//    };
//    ngtcp2_ssize send_datalen = ngtcp2_conn_write_connection_close(conn->quic->qconn,
//            (ngtcp2_path *)ngtcp2_conn_get_path(conn->quic->qconn),
//            NULL, dest_buf, destlen, &ccerr, uv_hrtime());
//
//}
...
//    settings.log_printf = debug_log_printf; /* lots of spam - enable when actually needed */
    ...
    uv_timer_start(&conn->quic->nudge_timer, nudge_timer_cb, 300, 300);
                                            ...
                                            size_t len, const char* data)
{
    ...
    if (!remote_sa) {
        remote_sa = (const struct sockaddr *)&_self->target;
    ...
    ret = ngtcp2_conn_read_pkt(conn->quic->qconn,
            &ps.path, &pi, (uint8_t*)data, len, uv_hrtime());
...
void _output_dnssim_quic_write_query(_output_dnssim_connection_t* conn, _output_dnssim_query_tcp_t* qry)
{
    ...
    int ret = ngtcp2_conn_open_bidi_stream(conn->quic->qconn, &qry->stream_id, NULL);
    if (ret) {
        lwarning("failed to open bidi stream: %s", ngtcp2_strerror(ret));
        return;
    }

    mlassert(qry->stream_id >= 0, "stream_id not assigned");
    quic_send_data(conn, NGTCP2_WRITE_STREAM_FLAG_NONE, vecs, 2, qry->stream_id);
