...
LINK_TYPES = {
    ...
    dpkt.pcap.DLT_RAW: lambda x: MockIP6Link(x),  # TODO can't handle IPv4 in DLT_RAW
}
...
def plot_client_query_scatter(ax, clients: Dict[bytes, int], color):
    ...
    lmax = 10
    while lmax <= max(data):
        samples = list(n for n in data if n >= lmin and n < lmax)
        ...
        logging.info(
            ...
            .format(lmin, lmax, len(samples), y[-1], s[-1]))
        lmin = lmax
        lmax *= 10

    logging.info(
        '  total: {:d} clients; {:d} queries'
        .format(len(data), sum(s)))
