...
def create_luaconfig(config: Dict[str, Any], threads: Dict[str, int], args: Any) -> str:
    ...
    try:
        ...
        if not args.read:
            raise RuntimeError("pcap must be set, use -r/--read")
        data['pcap'] = args.read
    ...
    if ips_per_thread == 0 and len(ips) > 0:  # less IPs than threads - share the same IPs
...
def assign_threads(config: Dict[str, Any], nthreads: int) -> Dict[str, int]:
    ...
    for _ in range(nthreads):
        ...
        rel_cpu_weights[chosen] = (
            (thread_cpu_weight * senders[chosen]['threads']) / senders[chosen]['target_cpu_weight'])
...
def main():
    ...
    parser.add_argument('-b', '--bind-net', type=str, nargs='*',
    ...
    parser.add_argument('-v', '--verbosity', help='verbosity level (0-5)', type=int, default=2)
    parser.add_argument('--dns-port', type=int, default=53, help='port for UDP/TCP DNS')
    parser.add_argument('--dot-port', type=int, default=853, help='port for DNS-over-TLS')
    parser.add_argument('--doh-port', type=int, default=443, help='port for DNS-over-HTTPS')
