...
-- split-clients.lua: separate clients (IPs) into multiple output files
--
...
for n = 1, args.noutputs do
	...
	if output:open(out_fname, LINKTYPE, SNAPLEN) ~= 0 then
		...
		log:notice("created output PCAP: " .. out_fname)
	end

	outputs[n] = { fn = out_fname, output = output, nclients = 0, npackets = 0 }
